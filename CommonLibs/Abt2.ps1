...
Start-Process -FilePath "C:\\TestManager\\\CommonLibs\\Abst64.exe" -ArgumentList $C -Wait -RedirectStandardOutput $tempfile -NoNewWindow
result_log (Get-Content $tempfile)
Start-Process -FilePath "C:\\TestManager\\\CommonLibs\\Abst64.exe" -ArgumentList $A -Wait -RedirectStandardOutput $tempfile -NoNewWindow
result_log (Get-Content $tempfile)
