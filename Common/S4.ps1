...
################## Main ##############################
$Parameter = '/sleep /c:1 /s:4 /d:30 /p:40'
try {
    process_log $PSCommandPath" is executing..."    
    $process = Start-Process -FilePath "C:\\TestManager\\Common\\pwrtest.exe" -ArgumentList $Parameter -Wait -PassThru -RedirectStandardOutput $tempfile
    $process.WaitForExit()
    result_log($tempfile)
